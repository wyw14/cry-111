...
func (s *ResetService) CommitAdjacentConcurrently(first, second ResetApproval) (ResetResult, error) {
	...
	commitHalf := func(approval ResetApproval, occupied bool) {
		<-start
		s.store.mu.Lock()
		section := s.store.sections[approval.SectionID]
		head := s.store.boundaries[section.Head]
		tail := s.store.boundaries[section.Tail]
		head.Baseline = head.Count
		tail.Baseline = tail.Count
		head.Revision++
		tail.Revision++
		s.store.boundaries[section.Head] = head
		s.store.boundaries[section.Tail] = tail
		section.Occupied = occupied
		section.Revision++
		s.store.sections[section.ID] = section
		s.store.mu.Unlock()
		done <- struct{}{}
	}
	go commitHalf(first, false)
	go commitHalf(second, true)
	close(start)
	<-done
	<-done
	return ResetResult{ApprovalID: first.ID, SectionIDs: []string{first.SectionID, second.SectionID}, Committed: true}, nil
}
